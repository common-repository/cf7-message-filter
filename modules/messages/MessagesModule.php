...
//		$this->module = 'packages';
	...
	public function updateDatabase( $data ) {
		...
		if ( trim( $data['email'] ) !== '' ) {
			...
			update_option( 'kmcfmf_email_stats', json_encode( $email_stats ) );
		}
	}
	 ...
	 * @deprecated
	 */
	public function getRows( $form_id = 0 ) {
	 ...
	 * @since 1.2.5.2
	 */
	public function getColumns( $form_id = 0 ) {
		$messages = $this->getMessages();
		$columns  = array();
		$messages = array_filter( $messages, function ( $val ) use ( $form_id ) {
			if ( property_exists( $val, 'id' ) ) {
	...
	function messagesPageContent() {
		if ( isset( $_GET['old'] ) ) {
			$this->wp_tools->renderView( 'messages.old' );
		} else if ( isset( $_GET['message_id'] ) ) {
			$this->wp_tools->renderView( 'messages.message' );
		} else {
			$this->wp_tools->renderView( 'messages.new' );
		}
	}
	...
	public function deleteMessage() {
		...
		if ( $validator->validate() && wp_verify_nonce( $nonce, 'can-delete-messages' ) ) {
			...
			foreach ( $message_ids as $message_id ) {
				...
				if ( $message ) {
					if ( ! $message->delete() ) {
						wp_send_json_error( __( "We could not delete this message", KMCF7MS_TEXT_DOMAIN ), 500 );
					}
				} else {
...
//					$contact_form->submit();
	...
	public function saveVisibleColumns() {
		...
		if ( $validator->validate() ) {
			...
			wp_send_json_success( __( "Visible columns saved", KMCF7MS_TEXT_DOMAIN ), 200 );
		}
		wp_die();
	}
